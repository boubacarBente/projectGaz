...
function InfoRow({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="px-8 py-5">
      ...
      <p className={`text-lg font-semibold ${accent ? 'text-primary' : 'text-base-content'}`}>{value}</p>
    </div>
  );
}
  ...
  return (
    <div className="max-w-4xl mx-auto print:mx-0">
      ...
      <div className="rounded-3xl border border-base-200/80 bg-base-100/80 shadow-lg shadow-black/5 backdrop-blur overflow-hidden print:shadow-none print:border print:border-base-200">
        {/* En-tête client */}
        <div className="px-8 pt-8 pb-6 border-b border-base-200/60">
          <div className="flex items-start justify-between">
            ...
            <div className="text-right">
              ...
              <p className="text-3xl font-bold text-primary mt-1">{formatCurrency(invoice.totalAmount)} <span className="text-sm text-base-content/40 font-normal">GNF</span></p>
            </div>
          </div>
        </div>
        ...
        {/* Métriques bénéfice */}
        {(cost > 0 || profit !== 0) && (
          <div className="grid grid-cols-3 divide-x divide-base-200/60 border-b border-base-200/60 bg-base-200/20">
            <div className="px-8 py-5 flex items-center gap-4">
              ...
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-base-content/40">Coût d&rsquo;achat</p>
                <p className="font-semibold">{formatCurrency(cost)} GNF</p>
              </div>
            </div>
            <div className="px-8 py-5 flex items-center gap-4">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                ...
                <svg className={`w-5 h-5 ${profit >= 0 ? 'text-success' : 'text-error'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 18L9 11.25l4.306 4.307a11.95 11.95 0 015.814-5.519l2.74-1.22m0 0l-5.94-2.28m5.94 2.28l-2.28 5.941" />
                </svg>
              </div>
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-base-content/40">Bénéfice</p>
                <p className={`font-semibold ${profit >= 0 ? 'text-success' : 'text-error'}`}>
            ...
            <div className="px-8 py-5 flex items-center gap-4">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                ...
                <svg className={`w-5 h-5 ${margin >= 0 ? 'text-info' : 'text-error'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 3v11.25A2.25 2.25 0 006 16.5h2.25M3.75 3h-1.5m1.5 0h16.5m0 0h1.5m-1.5 0v11.25A2.25 2.25 0 0118 16.5h-2.25m-7.5 0h7.5m-7.5 0l-1 3m8.5-3l1 3m0 0l.5 1.5m-.5-1.5h-9.5m0 0l-.5 1.5m.75-9l3-3 2.148 2.148A12.061 12.061 0 0116.5 7.605" />
                </svg>
              </div>
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-base-content/40">Marge</p>
                <p className={`font-semibold ${margin >= 0 ? 'text-info' : 'text-error'}`}>
                  {margin.toFixed(1)}%
                </p>
              </div>
            </div>
        ...
        {/* Notes */}
        {invoice.notes ? (
          <div className="px-8 py-5 border-t border-base-200/60 bg-base-200/20">
